...
string API::Call(bool authed, string path)
{
  return Call("GET", authed, path, "");
}

/* Checks if an Order is filled */
bool API::Is_Order_filled(string order_id)
{
...
string API::Place_Market_Order(string side, string size, string funds)
{
  string order_id = "";
  Document d;
  d.SetObject();
...
string API::Place_Stop_Order(string side, string price, string size, string funds)
{
  string order_id = "";
  Document d;
  d.SetObject();
...
string API::Get_AccountID(string currency)
{
  string ret = "";
...
string API::Get_AccountID(string currency, string &balance)
{
  string ret = "";
  string st = Call(true, "/accounts");
  Document d;
...
void API::Delete_Order(string order_id)
{
...
string API::Asset_Currency()
{
  return product_id.substr(0, 3);
}
string API::Fiat_Currency()
{
...
API::API()
{
  ...
  bool ex = exists(file);
  string s_uri, s_prod, s_socket;
  int tmp_interval;
  int tmp_perc_trans;
  Auth tmp;

  if (ex)
    Get_Conf(s_uri, s_socket, s_prod, tmp_interval, tmp_perc_trans, tmp);
  else
    Generate_Ex_Conf();
  ...
  percent_transact = tmp_perc_trans;
  auth = tmp;
